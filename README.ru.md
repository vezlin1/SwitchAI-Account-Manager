...
<p align="center">
  <img alt="Последний релиз" src="https://img.shields.io/github/v/release/vezlin1/SwitchAI-Account-Manager?label=version&color=18a8e8&style=flat-square">
...
## Поддерживаемые приложения и окружения

SwitchAI обеспечивает удобное переключение авторизации и отслеживание квот для всей экосистемы инструментов:

| Сервис | Поддерживаемые клиенты и окружения | Особенности интеграции |
| --- | --- | --- |
| **OpenAI / ChatGPT** | • **ChatGPT Desktop** (Windows и macOS)<br>• **Codex CLI** (`~/.codex/auth.json`) | Мгновенное переключение сессий, мониторинг квот, автоматический перезапуск приложения |
| **Google / Antigravity** | • **Antigravity 2.0**<br>• **Antigravity CLI** (`agy`)<br>• **Antigravity IDE** | Выбор целевых приложений для переключения, работа с защищённым системным хранилищем, детальные 5-часовые и недельные квоты |
...
Для Windows-релизов с тегом требуется секрет GitHub Actions
`TAURI_SIGNING_PRIVATE_KEY` (и при необходимости пароль). Открытый ключ,
встроенный в приложение, должен соответствовать этому закрытому ключу — тогда
`SwitchAI.exe.sig` и `latest.json` публикуются вместе. Если ключ не настроен,
сборка тегового релиза завершается ошибкой и не выпускает неполный автоапдейтер.
