...
pub fn dismiss_update_version_internal(
    ...
    commit_app_data(&mut current, next)?;
    let result_dto = AppDataDto::from(&*current);
    drop(current);
...
mod tests {
    ...
    fn check_for_updates_respects_auto_check_disabled() {
        ...
        let res = tauri::async_runtime::block_on(super::check_for_updates_internal(&state, false))
    ...
    fn dismissing_update_version_persists_and_clears_available_update() {
        ...
        *crate::app_state::lock_available_update(&state).unwrap() = Some(manifest);
        ...
        let res = super::dismiss_update_version_internal(&state, "9.9.9")
