...
The tagged release workflow signs `SwitchAI.exe`, generates `SwitchAI.exe.sig` and `latest.json`, and publishes those with the platform checksum files and macOS DMG. Minisign is separate from Windows Authenticode and Apple signing/notarization.
